...
import { Key, Table as TableIcon, Hash, Type } from 'lucide-react';
            ...
            <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 dark:bg-slate-800/50 text-xs uppercase text-slate-500 font-medium border-b border-slate-200 dark:border-slate-800">
                    <tr>
                        ...
                        <th className="px-6 py-3">长度</th>
                        <th className="px-6 py-3">允许空</th>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/50 text-xs uppercase text-slate-500 font-medium border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th className="px-6 py-3">索引名</th>
                            <th className="px-6 py-3">唯一</th>
                            <th className="px-6 py-3">定义</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {schema.indexes.map((idx) => (
                            <tr key={idx.name} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/50 text-xs uppercase text-slate-500 font-medium border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th className="px-6 py-3">约束名</th>
                            <th className="px-6 py-3">本表字段</th>
                            <th className="px-6 py-3">引用表</th>
                            <th className="px-6 py-3">引用字段</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {schema.foreignKeys.map((fk) => (
                            <tr key={fk.name} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                                <td className="px-6 py-4 font-mono text-slate-700 dark:text-slate-300">
                                    {fk.name}
                                </td>
