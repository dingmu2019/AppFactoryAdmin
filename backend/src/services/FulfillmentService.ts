...
import { CreditsService } from './CreditsService.ts';
...
export class FulfillmentService {
    ...
    private static async fulfillCredits(orderId: string) {
        ...
        if (!result.ok) {
            throw new Error(`Credits topup failed for order ${orderId}`);
        }
        console.log(`[Fulfillment] Credits granted: ${result.creditsGranted}`);
    }

    private static async fulfillSubscription(order: any, item: any) {
        // Example: If the order has a plan_key, ensure the user_app_relation is updated
        if (order.plan_key) {
            const { error } = await supabase
                .from('user_app_relations')
                .update({
                    subscription_status: 'active',
                    vip_level: order.plan_key,
            ...
            console.log(`[Fulfillment] Subscription activated for plan: ${order.plan_key}`);
        }
    }
}
