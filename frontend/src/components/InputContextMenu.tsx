...
export const InputContextMenu: React.FC = () => {
      ...
      switch (action) {
        ...
        case 'paste':
          ...
          try {
              ...
              showToast('无法访问剪贴板，请使用快捷键 Ctrl+V', 'error');
          }
          break;
      }
    } catch (err) {
  ...
  return (
    <div 
      ref={menuRef}
      className="fixed z-[9999] w-48 bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl py-1.5 animate-in fade-in zoom-in-95 duration-100 flex flex-col text-slate-200 text-sm font-medium select-none"
    ...
    >
      ...
      >
        <span className="flex items-center gap-3">
            <CheckSquare size={16} className="text-slate-400 group-hover:text-white transition-colors" />
            全选
        </span>
        <span className="text-xs text-slate-500 font-mono">{cmdKey}A</span>
      </button>
      ...
      >
        ...
        <span className="text-xs text-slate-500 font-mono">{cmdKey}C</span>
      </button>
      ...
      >
        <span className="flex items-center gap-3">
            <Scissors size={16} className="text-slate-400 group-hover:text-white transition-colors" />
            剪切
        </span>
        <span className="text-xs text-slate-500 font-mono">{cmdKey}X</span>
      </button>
      ...
      >
        <span className="flex items-center gap-3">
            <Clipboard size={16} className="text-slate-400 group-hover:text-white transition-colors" />
            粘贴
        </span>
        <span className="text-xs text-slate-500 font-mono">{cmdKey}V</span>
      </button>
    </div>
  );
};
