...
export default function AuditLogsPage() {
  ...
  useEffect(() => {
    setPageHeader(t('common.auditLog'), t('audit.subtitle') || 'System security and operation logs');
  }, [setPageHeader, t]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-start gap-4">
        ...
        <div className="flex gap-4">
            <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                ...
                <div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t('audit.todayStats')}</div>
            ...
            <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                ...
                <div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t('audit.failedStats')}</div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        <form onSubmit={handleSearch} className="flex flex-1 items-center gap-4 w-full">
          ...
          >
            <option value="">{t('audit.allApps') || '所有应用'}</option>
            {apps.map(app => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('audit.table.time')}</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">APP ID</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('audit.table.user')}</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('audit.table.action')}</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('audit.table.resource')}</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('audit.table.status')}</th>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.table.action')}</label>
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold uppercase tracking-wide
                ...
                <div className="flex-1">
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.table.resource')}</label>
                  <div className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 font-mono border border-slate-200 dark:border-slate-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.detail.requestId')}</label>
                  <p className="font-mono text-sm text-slate-700 dark:text-slate-300">{selectedLog.id}</p>
                </div>
                <div>
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.detail.timestamp')}</label>
                  <p className="font-mono text-sm text-slate-700 dark:text-slate-300">{formatDate(selectedLog.created_at)}</p>
                </div>
                <div>
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.table.user')}</label>
                  <p className="text-sm text-slate-700 dark:text-slate-300">{getUserLabel(selectedLog)}</p>
                  <p className="text-xs text-slate-400 font-mono mt-0.5">{selectedLog.user_id || ''}</p>
                </div>
                <div>
                  <label className="text-xs font-bold uppercase text-slate-400 mb-1 block">{t('audit.detail.clientInfo')}</label>
                  <p className="text-sm text-slate-700 dark:text-slate-300 font-mono">{selectedLog.ip_address}</p>
                  <p className="text-xs text-slate-400 truncate mt-0.5" title={selectedLog.user_agent}>{selectedLog.user_agent}</p>
                </div>
              </div>

              <div>
                <label className="text-xs font-bold uppercase text-slate-400 mb-2 block">{t('audit.detail.payload')}</label>
                <div className="bg-slate-50 dark:bg-slate-950 rounded-lg p-4 border border-slate-100 dark:border-slate-800 overflow-x-auto">
