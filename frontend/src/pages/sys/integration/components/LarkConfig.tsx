...
import { Eye, EyeOff, Save, Send, X, Globe } from 'lucide-react';
import type { LarkConfig } from '../../../../types/integration';
...
export const LarkConfigForm: React.FC<Props> = ({ initialData, initialEnabled, onSave, isSaving }) => {
  ...
  const [testContent, setTestContent] = useState(t('integration.lark.testContent'));
  ...
  const handleChange = (field: keyof LarkConfig, value: any) => {
    ...
    <div className="p-8 max-w-4xl mx-auto relative">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('integration.lark.title')}</h2>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="space-y-6 bg-white dark:bg-slate-900 rounded-xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">{t('integration.lark.appId')}</label>
                <input
                    type="text"
                    ...
                    placeholder={t('integration.lark.placeholder.appId')}
                />
            </div>

            <div className="space-y-2">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">{t('integration.lark.appSecret')}</label>
            ...
            <div className="space-y-2">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">{t('integration.lark.encryptKey')}</label>
                <input
                    type="text"
            ...
            <div className="space-y-2">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">{t('integration.lark.verificationToken')}</label>
                <input
                    type="text"
        ...
        <div className="flex items-center justify-end gap-4 pt-8 border-t border-slate-100 dark:border-slate-800 mt-8">
            ...
            <button
                onClick={() => onSave(formData as LarkConfig, isEnabled)}
                disabled={isSaving}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <h3 className="font-bold text-lg text-slate-900 dark:text-white">{t('integration.lark.title')} - {t('common.test')}</h3>
                    <button onClick={() => setShowTestModal(false)} className="text-slate-400 hover:text-slate-600"><X size={20}/></button>
                </div>
                <div className="p-6 space-y-4">
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">{t('integration.lark.recipient')}</label>
                        <input 
                            type="text" 
                            value={testUser}
                            onChange={e => setTestUser(e.target.value)}
                            placeholder={t('integration.lark.placeholder.recipient')}
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase">{t('integration.lark.content')}</label>
                        <textarea 
                            value={testContent}
