...
const CodeBlock: React.FC<CodeBlockProps> = ({ language, value }) => {
  ...
  return (
    <div className="relative group rounded-xl overflow-hidden my-4 border border-slate-200 dark:border-slate-800 bg-[#1e1e1e] !select-text" style={{ userSelect: 'text', WebkitUserSelect: 'text' }}>
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-2 bg-black/20 backdrop-blur-sm border-b border-white/5 opacity-0 group-hover:opacity-100 transition-opacity z-10">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{language || 'text'}</span>
        <button
          onClick={handleCopy}
