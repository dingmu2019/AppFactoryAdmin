...
export class ApiSyncService {
   ...
   * 
   * Strategy:
   * 1. Export static openapi.json.
   * 2. Iterate over all paths in swaggerSpec.
   * 3. For each path + method, upsert into DB.
   * 4. (Optional) Mark APIs not in swaggerSpec as inactive.
   */
  static async sync() {
    logger.info({ type: 'api-sync' }, 'Starting API Catalog Sync...');

    try {
      // 0. Export static openapi.json for AI tools and external docs
      if (!process.env.VERCEL) {
        const exportPath = path.resolve(__dirname, '../../../docs/openapi.json');
        const docsDir = path.dirname(exportPath);
        if (!fs.existsSync(docsDir)) {
            fs.mkdirSync(docsDir, { recursive: true });
        }
        fs.writeFileSync(exportPath, JSON.stringify(swaggerSpec, null, 2));
        logger.info({ path: exportPath }, 'Exported static OpenAPI definition file');
      }
