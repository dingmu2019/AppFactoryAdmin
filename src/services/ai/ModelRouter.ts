...
import { BaseAIProvider, ChatMessage, AIProviderConfig, AIProvider, ProviderConfig, AIRequest, AIResponse } from './providers/BaseAIProvider';
...
export class ModelRouter {
  ...
  registerProvider(config: ProviderConfig) {
    ...
    this.configs.sort((a, b) => (a.priority ?? 999) - (b.priority ?? 999));
  }
  ...
  async routeRequest(request: AIRequest): Promise<AIResponse> {
    console.log(`[ModelRouter] routeRequest called. Registered configs: ${this.configs.length}`);
    ...
    if (eligibleConfigs.length === 0) {
        const totalRegistered = this.configs.length;
