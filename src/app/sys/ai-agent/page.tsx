...
export default function AIAgentPage() {
  ...
  const fetchAgents = async (silent = false) => {
    ...
    try {
      ...
      showToast(`${t('agents.messages.loadFailed')}: ${error.message}`, 'error');
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(t('agents.messages.deleteSuccess'), 'success');
      fetchAgents(true);
    } catch (error: any) {
      console.error('Delete error:', error);
      showToast(`${t('agents.messages.deleteFailed')}: ${error.message}`, 'error');
    } finally {
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
        ...
        showToast(!currentStatus ? t('agents.messages.activated') : t('agents.messages.deactivated'), 'success');
    } catch (error: any) {
        console.error('Status update error:', error);
        showToast(`${t('agents.messages.statusUpdateFailed')}: ${error.message}`, 'error');
    }
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        ...
        showToast(`${t('agents.messages.reorderFailed')}: ${err.message}`, 'error');
    ...
    <div className="space-y-8 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-start gap-4">
        <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="bg-white dark:bg-zinc-900 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                 ...
                 <div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">{t('agents.total')}</div>
                    <div className="text-lg font-bold text-zinc-900 dark:text-white leading-none">{totalAgents}</div>
                 </div>
            </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                 ...
                 <div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">{t('agents.active')}</div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
            ...
            <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
            >
      ...
      {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-1">{t('agents.noAgents')}</h3>
