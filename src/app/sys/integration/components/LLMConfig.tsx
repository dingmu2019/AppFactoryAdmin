...
const SortableItem = ({ item, index, handleEdit, onDelete, configs }: any) => {
    ...
    return (
        <div 
            ref={setNodeRef} 
            style={style} 
            className={`bg-white dark:bg-zinc-900 p-5 rounded-lg border ${isDragging ? 'border-indigo-500 shadow-xl' : 'border-zinc-200 dark:border-zinc-800'} flex items-center justify-between group hover:border-indigo-500/50 transition-all shadow-sm mb-4`}
        >
            <div className="flex items-center gap-4">
                <button {...attributes} {...listeners} className="text-zinc-400 hover:text-indigo-600 cursor-grab active:cursor-grabbing">
                    <GripVertical size={20} />
                </button>
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-xs ${
                    item.is_enabled ? 'bg-indigo-600' : 'bg-zinc-400'
                }`}>
                    {conf.provider?.substring(0, 2).toUpperCase()}
                </div>
                <div>
                    <div className="flex items-center gap-2">
                        <h3 className="font-bold text-zinc-900 dark:text-white">{conf.model}</h3>
                        ...
                        {/* Disabled Label */}
                        {!item.is_enabled && (
                            <span className="text-[10px] px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 rounded-full border border-zinc-200 dark:border-zinc-700">
                                {t('common.disabled')}
                            </span>
                        )}
                    </div>
                    <div className="text-xs text-zinc-500 mt-1 flex items-center gap-3">
                        ...
                        <span>•</span>
                        <span className="font-mono opacity-70">{(conf.endpoint || (conf as any).baseUrl)?.replace(/^https?:\/\//, '')}</span>
                        {item.updated_at && (
                            <>
                                <span>•</span>
...
export const LLMConfigForm: React.FC<Props> = ({ configs, onSave, onDelete, onTest, isSaving, onSort }) => {
  ...
  const handleEdit = (config: IntegrationConfig) => {
    ...
    const conf = { ...defaultFormData, ...(config.config || {}) };
    // Handle mapping between baseUrl (seeded/legacy) and endpoint (UI)
    if (!conf.endpoint && (conf as any).baseUrl) {
        conf.endpoint = (conf as any).baseUrl;
    }
    setFormData(conf);
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-bold text-zinc-900 dark:text-white">{t('integration.llm.title')}</h2>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">{t('integration.llm.subtitle')}</p>
            </div>
        </div>
        <button
            onClick={handleAdd}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/30"
        >
            <Plus size={18} />
      ...
      <div className="grid gap-4">
        ...
        {configs.length === 0 && (
            <div className="text-center py-12 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700 text-zinc-500">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden relative border border-zinc-100 dark:border-zinc-800">
                {/* ... Header ... */}
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isEnabled ? 'bg-emerald-100 text-emerald-600' : 'bg-zinc-200 text-zinc-500'}`}>
                                <Play size={16} fill="currentColor" />
                            </div>
                            <div>
                                <div className="font-bold text-zinc-900 dark:text-white">{t('integration.llm.enableModel')}</div>
                                <div className="text-xs text-zinc-500">{t('integration.llm.enableDesc')}</div>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${formData.isPrimary ? 'bg-amber-100 text-amber-600' : 'bg-zinc-200 text-zinc-500'}`}>
                                <Globe size={16} />
                            </div>
                            <div>
                                <div className="font-bold text-zinc-900 dark:text-white">{t('integration.llm.setPrimary')}</div>
                                <div className="text-xs text-zinc-500">{t('integration.llm.primaryDesc')}</div>
                            </div>
                        </div>
                        <button
                            onClick={() => handleChange('isPrimary', !formData.isPrimary)}
                            className={`w-12 h-6 rounded-full transition-colors relative ${
                                formData.isPrimary ? 'bg-amber-500' : 'bg-zinc-300 dark:bg-zinc-700'
                            }`}
                        >
                            <div className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300">{t('integration.llm.provider')}</label>
                            <select
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300">{t('integration.llm.modelName')}</label>
                            <input
                                type="text"
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300">{t('integration.llm.apiKey')}</label>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300">{t('integration.llm.maxTokens')}</label>
                            <input
                                type="number"
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300">{t('integration.llm.temperature')}</label>
                            <input
                                type="number"
                                step="0.1"
                                min="0"
                ...
                <div className="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-end gap-3 bg-zinc-50 dark:bg-zinc-900">
                    <button
                        ...
                        className="px-4 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 font-medium hover:bg-white dark:hover:bg-zinc-800 transition-colors flex items-center gap-2 text-sm"
                    >
                        <Play size={16} />
                        {t('common.test')}
                    </button>
                    <button
                        ...
                        disabled={isSaving}
                        className="px-6 py-2 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/30 flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed text-sm"
                    >
                        {isSaving ? (
