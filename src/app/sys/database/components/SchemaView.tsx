    ...
    <div className="space-y-8 p-6">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <div className="flex items-start gap-4">
            ...
            <div>
                <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{schema.table.name}</h1>
                <p className="mt-1">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left whitespace-nowrap">
                    <thead className="bg-zinc-50 dark:bg-zinc-800/50 text-xs uppercase text-zinc-500 font-medium border-b border-zinc-200 dark:border-zinc-800">
                    <tr>
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                    {schema.columns.map((col) => (
                        <tr key={col.name} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50">
                ...
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                        {schema.indexes.map((idx, i) => (
                            <tr key={`${idx.name}-${i}`} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50">
                                ...
                                <td className="px-6 py-4 text-zinc-500 font-mono text-xs break-all">
                                    {idx.definition}
                                </td>
                            </tr>
                        ))}
      ...
      {schema.foreignKeys.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                <Key size={20} className="text-zinc-400 rotate-45" />
                ...
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                        {schema.foreignKeys.map((fk, i) => (
                            <tr key={`${fk.name}-${i}`} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50">
                                <td className="px-6 py-4 font-mono text-zinc-700 dark:text-zinc-300">
                                    {fk.name}
                                </td>
