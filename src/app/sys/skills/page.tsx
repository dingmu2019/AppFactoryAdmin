...
export default function SkillsPage() {
  ...
  const fetchSkills = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error || t('skills.errors.loadFailedDefault'));
      }
      const data = await res.json();
      setSkills(data);
    } catch (error: any) {
      console.error('Fetch error:', error);
      showToast(`${t('skills.errors.loadFailed')}: ${error.message}`, 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await authenticatedFetch(`/api/admin/skills/${skillToDelete}`, {
        method: 'DELETE'
      });
      
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || t('skills.errors.deleteFailedDefault'));
      }
      ...
      showToast(`${t('skills.errors.deleteFailed')}: ${error.message}`, 'error');
    } finally {
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      const res = await authenticatedFetch(`/api/admin/skills/${id}/status`, {
        method: 'PATCH',
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || t('skills.errors.statusUpdateFailedDefault'));
      }
      ...
      showToast(`${t('skills.errors.statusUpdateFailed')}: ${error.message}`, 'error');
    }
  };
    ...
    <div className="space-y-8 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-start gap-4">
        <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="bg-white dark:bg-zinc-900 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                 ...
                 <div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">{t('skills.total')}</div>
                    <div className="text-lg font-bold text-zinc-900 dark:text-white leading-none">{totalSkills}</div>
                 </div>
            </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-3 min-w-[140px]">
                 ...
                 <div>
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">{t('skills.active')}</div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
            <button className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
      ...
      {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-1">{t('skills.noSkills')}</h3>
