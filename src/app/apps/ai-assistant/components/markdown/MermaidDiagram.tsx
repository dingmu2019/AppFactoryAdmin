...
const MermaidDiagram: React.FC<MermaidDiagramProps> = ({ chart }) => {
  ...
  return (
    <div className={`relative group my-4 rounded-lg overflow-hidden transition-all ${isExpanded ? 'fixed inset-0 z-50 bg-white dark:bg-zinc-950 flex flex-col p-8' : 'border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900'}`}>
        ...
        <div className={`absolute top-3 left-3 right-3 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 ${isExpanded ? 'opacity-100' : ''}`}>
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className="p-2 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-500 hover:text-indigo-600 transition-all shadow-xl hover:scale-110"
                    title={viewMode === 'preview' ? t('common.ai.assistant.viewSource') : t('common.ai.assistant.viewChart')}
                >
                ...
                {viewMode === 'preview' && (
                    <button
                        ...
                        className="p-2 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-500 hover:text-indigo-600 transition-all shadow-xl hover:scale-110"
                        title={t('common.ai.assistant.copyAsImage')}
                    >
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className="p-2 bg-white/90 dark:bg-zinc-800/90 backdrop-blur-md border border-zinc-200 dark:border-zinc-700 rounded-lg text-zinc-500 hover:text-indigo-600 transition-all shadow-xl hover:scale-110"
                    title={isExpanded ? t('common.ai.assistant.collapse') : t('common.ai.assistant.fullscreen')}
                >
        ...
        >
            {viewMode === 'preview' ? (
                <div 
                    ref={containerRef}
                    className="mermaid-container flex items-center justify-center p-8 bg-white dark:bg-zinc-950 min-h-[120px] w-full"
